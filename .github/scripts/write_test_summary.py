...
def build_content(contents, mdFile, machine):
   ...
   mdFile.new_paragraph('\n')
   mdFile.write(f"<h4>RUNTIME: {(len(contents) - rt_fails)} / {len(contents)} had normal runtime.</h4>")
   ...
   mdFile.new_paragraph('\n')
   mdFile.new_paragraph('\n')
   mdFile.write(f"<h4>MEMORY: {(len(contents) - mem_fails)} / {len(contents)} had normal memory usage.</h4>")
