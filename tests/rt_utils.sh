...
function compute_petbounds_and_tasks_traditional_threading() {
  ...
  if [[ ${ATM_tasks:-0} -gt 0 ]]; then
     ...
     n=$((n + ATM_tasks))
     # shellcheck source=atm_omp_num_threads=1
  ...
  if [[ ${OCN_tasks:-0} -gt 0 ]]; then
     ...
     n=$((n + OCN_tasks))
     # shellcheck source=ocn_omp_num_threads=1
  ...
  if [[ ${ICE_tasks:-0} -gt 0 ]]; then
     ...
     n=$((n + ICE_tasks))
     # shellcheck source=ice_omp_num_threads=1
  ...
  if [[ ${WAV_tasks:-0} -gt 0 ]]; then
     ...
     n=$((n + WAV_tasks))
     # shellcheck source=wav_omp_num_threads=1
...
function compute_petbounds_and_tasks_esmf_threading() {
  ...
  if [[ ${lnd_model:-} = "lm4" ]]; then
      ...
      # shellcheck source=lnd_omp_num_threads=1
  ...
  # FBH
  if [[ ${FBH_tasks:-0} -gt 0 ]]; then
     # shellcheck source=fbh_omp_num_threads=1
